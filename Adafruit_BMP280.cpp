...
void Adafruit_BMP280::sleep() {
  byte ctrl=read8(BMP280_REGISTER_CONTROL);
  ctrl &= 0xFC;
  write8(BMP280_REGISTER_CONTROL, ctrl);
}

void Adafruit_BMP280::forcedMode() {
  byte ctrl=read8(BMP280_REGISTER_CONTROL);
  ctrl &= 0xFC;
  ctrl |= 0x1;
  write8(BMP280_REGISTER_CONTROL, ctrl);
...
void Adafruit_BMP280::normalMode() {
  byte ctrl=read8(BMP280_REGISTER_CONTROL);
  ctrl &= 0xFC;
  ctrl |= 0x3;
  write8(BMP280_REGISTER_CONTROL, ctrl);
...
void Adafruit_BMP280::setPressureOversampling(byte os)
{
  byte ctrl=read8(BMP280_REGISTER_CONTROL);
  ctrl &= 0xE3;
  ctrl |= (os << 2);
...
void Adafruit_BMP280::setTemperatureOversampling(byte os)
{
  byte ctrl=read8(BMP280_REGISTER_CONTROL);
  ctrl &= 0x1F;
  ctrl |= (os << 5);
